...
import { getAuth } from "@clerk/express"; // <-- SHU QATORNI QO'SHING
...
// HELPER FUNTIONS
// this funtion will return finite number
...
// this function will ger the user from clerl and return yser details
...
// to createe a serviceAppointment
  ...
  try {
    ...
    if (!clerkUserId)
      return res.status(401).json({
        success: false,
        message: "Authencation is required to create a service appointment",
      });
  ...
  try {
    ...
    try {
      ...
      return res.status(404).json({
        success: false,
        message: "Stripe session no found",
      });
    }
    if (!session) {
      return res.status(404).json({
