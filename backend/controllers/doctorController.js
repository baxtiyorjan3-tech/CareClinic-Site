...
// this function will convert doctor data in a plain text
...
export async function updateDoctor(req, res) {
  try {
    ...
    // if exits then update the image else show warnign
    if (req.file?.path) {
      const uploaded = await uploadToCloudinary(req.file.path, "doctors");
      if (uploaded) {
...
export async function deleteDoctor(req, res) {
  try {
    ...
    return res.json({ succes: true, message: "Doctor Removed" });
  } catch (err) {
...
export async function toggleAvailability(req, res) {
  try {
    ...
    if (!req.doctor || String(req.doctor._id || req.doctor.id) !== String(id)) {
      return res.status(403).json({
        success: false,
        message: "Not authorized to update this doctors availability",
      });
    }
    ...
    await doc.save();
    const out = normalizeDocForClient(doc.toObject());
    delete out.passwordl;
    return res.json({ succes: true, data: out });
  } catch (err) {
